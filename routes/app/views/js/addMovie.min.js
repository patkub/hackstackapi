...
$(function () {
  ...
  (function (hackstack) {
    ...
    $("#imdbID").on("input", function (e) {
      // clear movie search
      $("#movieSearch").val(""); // extract imdb id, supported formats:
      // 1) https://www.imdb.com/title/tt1502397/
      // 2) tt1502397

      var imdb_id = $("#imdbID").val().match(/tt\d+/); // http://www.omdbapi.com/?apikey=[yourkey]&

      var omdb_url = "http://www.omdbapi.com/?apikey={0}&i={1}".format(hackstack.API_KEYS.OMDB, imdb_id);
      ...
      $("#movieSearch").autocomplete({
        source: function source(request, response) {
          ...
          return $.ajax({
            ...
            url: "http://www.omdbapi.com/?apikey={0}&s={1}&type=movie&r=json".format(hackstack.API_KEYS.OMDB, search),
            dataType: "json",
    ...
    $("#movieSearch").on("autocompleteselect", function (e, ui) {
      ...
      var imdbID = ui.item.value; // http://www.omdbapi.com/?apikey=[yourkey]&

      var omdb_url = "http://www.omdbapi.com/?apikey={0}&i={1}".format(hackstack.API_KEYS.OMDB, imdbID);
      $.getJSON(omdb_url, function (data) {
        return fillFormData(data);
      });
    });
    ...
    $("#addNewMovie").submit(function () {
      ...
      console.log(data); // it will return a boolean with whether or not the item was added

      $.post("http://192.168.122.1:8080/movies/add", data).done(function (msg) {
        // successfully added
        $("#alert").removeClass("d-none").addClass("alert-success").html("<strong>Movie added successfully!</strong>");
      }).fail(function (xhr, textStatus, errorThrown) {
        // failed to add
        $("#alert").removeClass("d-none").addClass("alert-danger").html("<strong>Oh no! An error occurred trying to add the movie.</strong>");
